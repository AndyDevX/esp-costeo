        ...
        function handleInput() {
            ...
            if (ingresoAPF.value || ingresoAPM.value) {
                ...
                if (deduccionOpcional === 'si') {
                    ...
                    document.getElementById('i_p').disabled = false; // Habilitar i_p
                    document.getElementById('i_p').required = true;  // Hacer i_p requerido
                }
...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $año = isset($_POST['año']) ? $_POST['año'] : 0;
       ...
       // echo "la base gravable: $base_gravable_m <br>";
       ...
       /* ...  $busqueda_id = "SELECT id FROM tarifa WHERE ? BETWEEN limite_inferior AND limite_superior";
        ...
        $limite_inferior = "SELECT limite_i FROM tarifa WHERE ? BETWEEN year_i AND year_f AND ? BETWEEN limite_i AND limite_s AND periodo=1 ";

        $stmt = $connection->prepare($limite_inferior);
        if ($stmt === false) {
            die("Error en la preparación de la consulta: " . $connection->error);
        }
// $stmt->bind_param("d", $resultado) la "d" indica float y la "i" entero
        $stmt->bind_param("ii", $año,$base_gravable_m);
        $stmt->execute();
        $stmt->bind_result($li);
        ...
        $tasa = "SELECT por_ciento  FROM tarifa WHERE ? BETWEEN year_i AND year_f AND ? BETWEEN limite_i AND limite_s AND periodo=1 ";

        $stmt = $connection->prepare($tasa);
        if ($stmt === false) {
            die("Error en la preparación de la consulta: " . $connection->error);
        }
        ...
        $cuota_fija = "SELECT cuota_fija  FROM tarifa WHERE ? BETWEEN year_i AND year_f AND ? BETWEEN limite_i AND limite_s AND periodo=1 ";

        $stmt = $connection->prepare($cuota_fija);
        if ($stmt === false) {
            die("Error en la preparación de la consulta: " . $connection->error);
        }
       ...
       // echo ": $resultado <br>";
        ...
        $pago_mes = $impuesto-$retención_isr;
    ?>
        <h2>Resultados de la base de datos:</h2>
        <label for="resultado1">Ingresos del mes:</label>
        <input type="text" id="resultado1" value="<?php echo number_format($ingreso_mes, 2, '.', ','); ?>" readonly><br><br>
        <label for="resultado1">Deduccioens del mes:</label>
        <input type="text" id="resultado1" value="<?php echo number_format($deduccion_mes, 2, '.', ','); ?>" readonly><br><br>
        ...
        <input type="text" id="resultado9" value="<?php echo number_format($pago_mes, 2, '.', ','); ?>" readonly><br><br>
